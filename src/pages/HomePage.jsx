...
const ROLES = [
  { num: "01", title: "Product", desc: "You ship a thing people use.", tags: "PM · DESIGNER · ENG" },
  { num: "02", title: "Service", desc: "You deliver work for clients.", tags: "CONSULTING · AGENCY · OPS" },
    ...
    >
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
        <span className="text-[#A8C888] text-sm font-light tracking-wider">
          {file ? file.name.slice(0, 18) + (file.name.length > 18 ? "…" : "") : "click or drop"}
        </span>
        <span
...
function SmallFlower({ x, y, size = 1 }) {
  return (
    <svg
      style={{ position: "absolute", top: y, left: x, width: 50 * size, height: 90 * size }}
      viewBox="0 0 50 90"
    >
...
function StemFlower({ x, y }) {
  return (
    <svg style={{ position: "absolute", top: y, left: x, width: 28, height: 72 }} viewBox="0 0 28 72">
      <line x1="14" y1="72" x2="14" y2="22" stroke="#7A9A5A" strokeWidth="1.5"/>
...
function LollipopFlower({ x, y }) {
  return (
    <svg style={{ position: "absolute", top: y, left: x, width: 38, height: 88 }} viewBox="0 0 38 88">
...
export default function HomePage() {
  ...
  const handleFlowerClick = () => fileInputRef.current?.click();
  ...
  const handleScore = async () => {
    if (!file) { setError("Drop a PDF into the flower first ↗"); return; }
  ...
  return (
    <div className="flex flex-1 overflow-hidden" style={{ height: "calc(100vh - 56px)" }}>
      <div className="w-1/2 flex flex-col px-12 pt-10 pb-6 overflow-y-auto" style={{ background: "#EDE8DC" }}>
        ...
        <h1 style={{ fontSize: "2.85rem", fontWeight: 800, color: "#1B3320", lineHeight: 1.15 }} className="mb-3">
          Plant your{" "}
          <em style={{ fontFamily: "'Playfair Display', serif", fontWeight: 400, fontStyle: "italic", color: "#1B3320" }}>
            résumé.
          </em>
          <br />
          Watch it grow.
        ...
        <p style={{ fontSize: 10, fontWeight: 700, letterSpacing: "0.15em", color: "#8A9A8A" }} className="mb-4">
          CHOOSE A ROLE TYPE
        </p>
        ...
        <div className="flex gap-3 mb-auto">
          {ROLES.map((role, i) => {
            ...
            return (
              ...
              >
                <div style={{ fontSize: 11, fontWeight: 700, color: active ? "#C4963C" : "#A0AEA0", marginBottom: 6 }}>
                  {role.num}
                </div>
                <div style={{ fontWeight: 700, fontSize: "0.95rem", color: active ? "#FFFFFF" : "#1B3320", marginBottom: 4 }}>
                  {role.title}
                </div>
                <div style={{ fontSize: "0.75rem", color: active ? "#A0B8A0" : "#7A8A7A", marginBottom: 12 }}>
                  {role.desc}
                </div>
                <div
                  style={{
        ...
        <div className="flex items-center gap-4 mt-8">
          ...
          >
            <span style={{ fontWeight: 600, fontSize: "0.95rem", color: "#1B3320" }}>
              {loading ? "Scoring…" : "Score it"}
            </span>
            <span style={{ background: "#FFFFFF", borderRadius: 10, padding: "6px 10px", fontSize: "0.9rem", color: "#1B3320" }}>
              →
            </span>
          </button>
          {error ? (
            ...
            <p style={{ color: "#3A7A3A", fontSize: "0.82rem", fontWeight: 500 }}>✓ {file.name}</p>
          ) : (
            <p style={{ color: "#9AAA9A", fontSize: "0.82rem" }}>Drop a PDF into the flower first ↗</p>
          )}
        </div>
      </div>

      <div className="w-1/2 relative flex items-center justify-center overflow-hidden" style={{ background: "#C5DBA8" }}>
        ...
        <div className="absolute bottom-0 left-0 right-0" style={{ height: 2, background: "#B0C898" }}>
          <div style={{ width: "33%", height: "100%", background: "#4A7A3A" }} />
        </div>
      </div>
    </div>
  );
